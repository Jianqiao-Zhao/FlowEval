...
if __name__ == "__main__":
    # you might also need to manually set "split_allocation_all.json" to "split_allocation.json" at line 86, model.py
    train_path = "training_set"
    test_path = "test_sets/human-to-bot"
    model_path = "IntentBERT/daily/models_daily"
    segment = [0, 91, 187, 278]
    ...
    train_intent = load_intent(train_path, mode='train')
    train_utterance = load_utterance(train_path, mode='train')
    train_intent_features = extract_intent_features(train_path, model_path, data_mode='train', pooler_type=pooler_type, overwrite=recompute)
    train_utterance_features = extract_utterance_features(train_path, mode='train', overwrite=recompute)
    ...
    human_bot_reference_score = {
        "overall": [
            np.mean(np.asarray([num for num in v["overall"] if isinstance(num, int) or isinstance(num, float)]))
            if isinstance(v["overall"], list) else v["overall"]
            for v in json.load(open(os.path.join(test_path, "dialog_id_to_score.json"))).values()
        ],
    }
